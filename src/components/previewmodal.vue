<template>
  <div
    v-if="prevshow"
    class="absolute top-0 left-0 w-screen h-screen flex items-center"
    style="background-color:rgba(0, 0, 0, 0.7);"
  >
    <div class="w-full">
      <div class="relative mx-auto px-24 py-16 w-1/3 bg-white rounded-t">
        <svg
          @click="imagerotate"
          class="top-0 right-0 mt-2 mr-2 absolute w-6 fill-current text-blue-600 cursor-pointer"
          viewBox="0 0 22 22"
        >
          <path
        ...
        <img v-if="previewimage !== ''" class="rounded" :src="previewimage" alt />
      </div>
      <div
        class="mx-auto border-t border-blue-500 rounded-b w-1/3 h-16 bg-white flex justify-center"
      >
        <button
          @click="changepreviewshow"
          class="mr-20 px-4 my-2 font-bold text-blue-600 rounded hover:bg-blue-300"
        >取消</button>
        <button
          @click="confirmpreview"
          class="ml-20 px-4 my-2 font-bold text-blue-600 rounded hover:bg-blue-300"
        >确认</button>
      </div>
    </div>
  </div>
...
export default {
  computed: {
    ...mapGetters(["prevshow", "previewimage", "previewuploadfile"])
  },
  data() {
    return {
      show: false,
